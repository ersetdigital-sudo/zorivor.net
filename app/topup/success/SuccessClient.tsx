...
export function SuccessClient({
  ...
  const inv = invoice || fallbackInvoice;
  ...
  const price = order?.amount_idr ?? 0;
  const fee = 0;
  const total = price + fee;
  const nominal = order?.denomination ?? "—";
  const game = order?.game ?? "Mobile Legends: Bang Bang";
  const methodLabel = paymentMethodLabel ?? order?.payment_method ?? "QRIS";
  const methodImage = paymentMethodImage;
  const showQris = qrisUrl && (methodLabel === "QRIS" || !methodImage);
      ...
      <main className="mx-auto max-w-3xl px-5 py-12 md:py-20">
        ...
        <div className="text-center">
          <div className="relative mx-auto mb-6 size-20">
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle, rgba(94,234,212,.4), transparent 60%)",
                filter: "blur(8px)",
              }}
            />
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, rgba(94,234,212,.2), rgba(167,139,250,.15))",
          ...
          <p className="mx-auto mt-3 max-w-md text-sm text-white/60 md:text-base">
            {showQris
              ? "Scan QRIS di bawah untuk menyelesaikan pembayaran. Diamond akan masuk otomatis setelah pembayaran terkonfirmasi."
              : "Diamond akan masuk ke akun kamu otomatis. Biasanya kurang dari 9 detik."}
          </p>
        </div>
        ...
        <div className="mt-10 overflow-hidden rounded-3xl border border-white/8 bg-white/[0.02] backdrop-blur">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, rgba(124,92,255,.18), rgba(34,225,196,.08))",
          ...
          >
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle, rgba(167,139,250,.4), transparent 60%)",
                filter: "blur(20px)",
              }}
            />
            <div className="relative flex items-center gap-4">
              <div className="grid size-14 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-violet-500 to-blue-500 text-sm font-bold text-white">
                {gameInitial(game)}
              </div>
              <div className="min-w-0 flex-1">
                <div className="truncate text-base font-semibold">{game}</div>
                <div className="text-xs text-white/45">{nominal}</div>
              </div>
          ...
          {/* Body */}
          <div className="p-5">
            {/* Payment method image (logo or QRIS) */}
            {(showQris || methodImage) && (
              <div className="mb-5 flex flex-col items-center gap-2">
                <div className="text-[11px] uppercase tracking-wider text-white/45">
                  {showQris ? "Scan QRIS" : `Logo ${methodLabel}`}
                </div>
                <div
                  className="overflow-hidden rounded-2xl border border-white/10 bg-white shadow-[0_0_0_1px_rgba(0,0,0,0.05)]"
                  style={{
                    width: 240,
                    height: showQris ? 288 : 80,
                    maxWidth: "100%",
                  }}
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={showQris ? qrisUrl! : methodImage!}
                    alt={methodLabel}
            ...
            <div className="space-y-3 text-sm">
              <Row label="Nominal" value={nominal} />
              <Row
                label="Metode bayar"
                value={
                  <span className="flex items-center gap-2">
                    {methodImage && !showQris ? (
                      /* eslint-disable-next-line @next/next/no-img-element */
                      <img
                        src={methodImage}
                        alt=""
            ...
            <div className="space-y-2 text-sm">
              ...
              {order?.cashback_idr ? (
                <div className="flex items-center justify-between text-emerald-300">
                  <span>Cashback</span>
                  <span>+ {formatRp(order.cashback_idr)}</span>
                </div>
              ) : null}
            </div>
            ...
            <div className="mt-4 flex items-end justify-between border-t border-white/8 pt-4">
              ...
              <span className="dis text-2xl font-extrabold">
                {formatRp(total)}
              </span>
            </div>
          </div>
        ...
        <div className="mt-6 rounded-3xl border border-white/8 bg-white/[0.02] p-5 backdrop-blur">
          ...
          <ul className="space-y-2.5 text-sm text-white/70">
            <li className="flex gap-3">
              ...
              <span>
                Cek in-game dulu, biasanya item sudah masuk sebelum kamu sampai
                ke sini.
              </span>
            </li>
            <li className="flex gap-3">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          <a
            href="/"
            className="btn flex-1 rounded-2xl px-6 py-3.5 text-sm font-semibold"
          >
            <svg
              width="14"
              height="14"
...
function gameInitial(game: string) {
  const m = game.match(/\b([A-Z])/g);
  return (m ? m.slice(0, 2).join("") : game.slice(0, 2)).toUpperCase();
}
