...
type Hint = "string" | "number" | "boolean" | "json" | "string[]";

const TYPE_HINTS: Record<string, Hint> = {
  support_whatsapp: "string",
  site_name: "string",
  hero_title: "string",
};

const DEFAULT_SETTINGS: Array<{ key: string; value: string; type: Hint }> = [
  {
  ...
  {
    key: "hero_title",
    value: "Top Up Game Termurah, Tanpa Biaya Admin",
    type: "string",
  },
];
...
export function SettingsEditor({ settings }: { settings: Setting[] }) {
  ...
  const list = settings.length > 0 ? settings : DEFAULT_SETTINGS.map((d) => ({
    key: d.key,
    value: d.value,
    updated_at: new Date(0).toISOString(),
  }));

  async function save(key: string, rawValue: string, hint: Hint) {
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {list.map((s) => {
          const hint = TYPE_HINTS[s.key] ?? "string";
          const isCustom = byKey.has(s.key);
          return (
            <SettingCard
              key={s.key}
              setting={s}
              ...
              onRemove={isCustom ? () => remove(s.key) : undefined}
            />
          );
        })}
        {list.length === 0 && (
...
function SettingCard({
  ...
  const display = prettyValue(setting.value, hint, setting.key);
    ...
    return (
      <div className="rounded-2xl border border-violet-400/30 bg-violet-500/5 p-5">
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") onSave(raw).then((ok) => ok && setEditing(false));
            if (e.key === "Escape") {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
      ...
      <div className="rounded-lg border border-white/5 bg-black/40 px-3 py-2 text-sm text-white/90">
        {display}
      </div>
      <div className="mt-2 flex justify-end gap-2">
        <button
...
function parseInput(raw: string, hint: Hint): unknown {
...
function rawForEdit(v: unknown, hint: Hint): string {
  if (hint === "string" || hint === "number") {
    return v == null ? "" : String(v);
  }
  if (hint === "boolean") return v ? "true" : "false";
  if (hint === "string[]") return Array.isArray(v) ? v.join("\n") : "";
  return v == null ? "" : JSON.stringify(v, null, 2);
}

function prettyValue(v: unknown, hint: Hint, key: string): React.ReactNode {
  if (key === "support_whatsapp" && typeof v === "string") {
    return (
